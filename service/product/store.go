...
func (s *Store) GetProductsByID(productID int) (*types.Product, error) {
...
func (s *Store) GetProductByIDs(productIDs []int) ([]types.Product, error) {
	...
	// Args are usually of type interface
...
func (s *Store) UpdateProduct(product types.Product) error {
	_, err := s.db.Exec("UPDATE products SET name = ?, decription = ?, image = ?, price = ?, quantity = ? WHERE id = ?", product.Name, product.Description, product.Image, product.Price, product.Quantity, product.ID)
	if err != nil {
		return err
	}
	return nil
